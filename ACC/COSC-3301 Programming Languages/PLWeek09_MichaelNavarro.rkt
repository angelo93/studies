#lang racket
;-------------------- Start Binary Tree Sort --------------------
; This assumes the tree passed in is a sorted binary tree
(define (tree-to-list tree)
  (cond
    ;-------------------- Start Break Cases --------------------
    ; Case 1: Tree/leaf is empty
    [(empty? tree)
     '()]
    ; Case 2: Leaf is childless
    [(empty? (rest tree))
     (list (car tree))]
    ;-------------------- Start Recursion --------------------
    [else
     (append (tree-to-list (second tree)) ; Traverse left side of tree
             (list (first tree))          ; Append root on our way back up
             (tree-to-list (third tree))  ; Traverse right side of tree
             )]
    )
  )
...
(define (list-to-tree num_list binary_tree)
  ...
  (append result temp_tree)
  )
; End list-to-tree

;-------------------- Start Tests --------------------

(display "Test Case List: (22 25 7 16 8 34 67 7 32 17 8 4  5 3)\n")
